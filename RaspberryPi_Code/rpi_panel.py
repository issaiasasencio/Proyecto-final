...
class RPiOperatorPanel(ctk.CTk):
    def __init__(self):
        ...
        default_model = self.config_data.get("active_model", os.path.join(self.modelos_dir, "best_ncnn_model"))
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(self.sidebar, text="OPERACIÓN", font=ctk.CTkFont(size=14, weight="bold"), text_color="#1E88E5").pack(pady=(20,10))

        self.btn_power = ctk.CTkButton(self.sidebar, text="ENCENDER SCANNER", fg_color="#2E7D32", hover_color="#1B5E20", 
                                      font=ctk.CTkFont(weight="bold"), height=50, command=self.toggle_scanner)
        ...
        ctk.CTkLabel(self.sync_frame, text="ÚLTIMO RELEVAMIENTO (SSH)", font=ctk.CTkFont(size=11, weight="bold")).pack(pady=5)
        ...
        for i in range(1, 5):
            ...
            lbl_asig = ctk.CTkLabel(f, text="(Sin asignar)", font=ctk.CTkFont(size=11, slant="italic"), text_color="#AAAAAA")
            lbl_asig.pack()
    ...
    def update_stats_loop(self):
        ...
        if self.engine.running:
            ...
            for index, is_occ in enumerate(occupied):
                if not is_occ:
                    self.servo_labels[index].configure(text="LIBRE", text_color="#666666")
...
class SettingsDialog(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        ctk.CTkButton(self, text="Guardar y Aplicar", fg_color="#2E7D32", command=self.apply).pack(pady=30)
...
class HistoryDialog(ctk.CTkToplevel):
    ...
    def load_models(self):
        ...
        items = [d for d in os.listdir(base_dir) if d.endswith("_ncnn_model") or d.endswith(".pt")]
        ...
        for m in items:
            ...
            # Metadatos
            meta_path = m_path.replace(".pt", ".json") if m.endswith(".pt") else m_path.rstrip("/") + ".json"
        ...
        for data in model_data_list:
            ...
            # Tarjeta de Modelo
            f = ctk.CTkFrame(self.scroll, corner_radius=10, border_width=1, border_color="#333333")
            ...
            ctk.CTkLabel(top_row, text=m, font=ctk.CTkFont(size=13, weight="bold"), text_color="#1E88E5").pack(side="left")
            btn = ctk.CTkButton(top_row, text="Activar", width=80, height=24, fg_color="#1E88E5", hover_color="#1565C0",
