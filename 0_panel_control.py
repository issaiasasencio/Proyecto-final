...
class SourceSelectorDialog(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        lbl = ctk.CTkLabel(
            self,
            text="Selecciona la camara o video a utilizar:\n(¿Desde donde ingresaremos el objeto?)",
        ...
        btn_rpi = ctk.CTkButton(
            frame,
            text="1. Camara Inalambrica (Raspberry Pi)",
            font=ctk.CTkFont(size=14, weight="bold"),
            fg_color="#00897B",
            hover_color="#00695C",
            height=50,
        ...
        btn_pc = ctk.CTkButton(
            frame,
            text="2. Camara Local (PC o Iriun)",
            font=ctk.CTkFont(size=14, weight="bold"),
            fg_color="#1E88E5",
            hover_color="#1565C0",
            height=50,
...
class HistoryDialog(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        header = ctk.CTkLabel(
            self, text="Modelos Archivados", font=ctk.CTkFont(size=20, weight="bold")
        )
        header.grid(row=0, column=0, pady=20)

        # Scrollable list
        self.scroll_frame = ctk.CTkScrollableFrame(
            self, label_text="Versiones Guardadas"
        )
    ...
    def load_history(self):
        ...
        files.sort(
            key=lambda x: os.path.getmtime(os.path.join(self.archive_dir, x)),
            reverse=True,
        )

        if not files:
            ctk.CTkLabel(self.scroll_frame, text="No hay modelos guardados aún.").pack(
                pady=20
            )
            return
        ...
        for f in files:
            ...
            mtime = time.strftime(
                "%d/%m/%Y %H:%M", time.localtime(os.path.getmtime(f_path))
            )
    ...
    def activate_model(self, path):
        try:
            ...
            messagebox.showinfo(
                "Éxito", f"Modelo activo cambiado a:\n{os.path.basename(path)}"
            )
            self.parent.log(
...
class ReportDialog(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        self.results_path = os.path.join(
            project_abs, "modelo_produccion", "results.csv"
        )
        ...
        ctk.CTkLabel(
            self, text="Reporte de Desempeño", font=ctk.CTkFont(size=22, weight="bold")
        ).pack(pady=20)
    ...
    def load_report(self):
            ...
            try:
                with open(self.results_path, mode="r", encoding="utf-8") as f:
                    ...
                    if reader:
                        ...
                        val_key = next(
                            (k for k in last_row.keys() if "mAP50(B)" in k), None
                        )
                        if val_key:
        ...
        if score_pct >= 90:
            status, color, msg = (
                "EXCELENTE",
                "#4CAF50",
                "Precision optima. El sistema esta listo para produccion.",
            )
        elif score_pct >= 70:
            status, color, msg = (
                "ACEPTABLE",
                "#FBC02D",
                "Funcionamiento estable. Podria presentar variaciones menores.",
            )
        else:
            status, color, msg = (
                "INSUFICIENTE",
                "#F44336",
                "Se requiere mayor volumen de datos para entrenamiento.",
            )
        ...
        gauge = ctk.CTkProgressBar(
            self.content_frame, height=20, progress_color=color, fg_color="#333333"
        )
        gauge.set(map50)
        ...
        ctk.CTkLabel(
            self.content_frame,
            text=f"Puntaje de Precisión: {score_pct:.1f}%",
            font=ctk.CTkFont(size=14),
        ).pack()
        ...
        ctk.CTkLabel(
            msg_frame, text=msg, wraplength=350, font=ctk.CTkFont(size=13)
        ).pack(pady=10)
...
class TrainingManagerDialog(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        self.selected_categories = {
            cat: tk.BooleanVar(value=True) for cat in self.categorias
        }
        self.servo_vars = {
            cat: tk.StringVar(value=self.mapping.get(str(i), "1"))
    ...
    def setup_ui(self):
        ...
        desc = ctk.CTkLabel(
            self,
            text="Seleccioná qué objetos incluir y asignales su brazo robótico:",
        ...
        self.scroll = ctk.CTkScrollableFrame(
            self, label_text="Objetos en Base de Datos", height=280
        )
        ...
        for i, cat in enumerate(self.categorias):
            ...
            cb = ctk.CTkCheckBox(
                f,
                text=cat.upper(),
                variable=self.selected_categories[cat],
                font=ctk.CTkFont(size=14, weight="bold"),
                width=150,
            ...
            # Servo Selector
            ctk.CTkLabel(f, text="Brazo:", font=ctk.CTkFont(size=12)).pack(
                side="left", padx=(10, 5)
            )
            om = ctk.CTkOptionMenu(
                f,
                values=["1", "2", "3", "4"],
                variable=self.servo_vars[cat],
                width=80,
                fg_color="#333333",
                button_color="#444444",
                command=lambda _: self.update_validation(),
            )
        ...
        ctk.CTkLabel(
            mode_frame,
            text="Protocolo de Red Neuronal:",
            font=ctk.CTkFont(weight="bold"),
        ).pack(pady=(10, 5))

        r1 = ctk.CTkRadioButton(
            mode_frame,
            text="Evolución Continua (Usa experiencia previa)",
        ...
        r2 = ctk.CTkRadioButton(
            mode_frame,
            text="Red Limpia (Olvida todo y aprende de cero)",
            variable=self.train_mode,
            value="scratch",
            text_color="#FF9800",
        )
    ...
    def update_validation(self):
        selected_cats = [
            cat for cat in self.categorias if self.selected_categories[cat].get()
        ]
        ...
        if count == 4 and not duplicados:
            self.btn_start.configure(
                state="normal", fg_color="#1E88E5", text="CONTINUAR ENTRENAMIENTO"
            )
        elif duplicados:
            self.btn_start.configure(
                state="disabled", fg_color="#555555", text="ERROR: BRAZOS DUPLICADOS"
            )
        else:
            self.btn_start.configure(
                state="disabled",
                fg_color="#555555",
                text=f"SELECCIONÁ 4 OBJETOS ({count}/4)",
            )

    def delete_category(self, category):
        if not messagebox.askyesno(
            "Confirmar Borrado",
            f"¿Estás seguro de eliminar '{category.upper()}' de la base de datos?\n\nEsta acción es irreversible y borrará todas sus fotos.",
            parent=self,
        ):
            return
        ...
        try:
                ...
                for file in files:
                    if file.endswith(".txt"):
                        ...
                        if lines_to_keep:
                            ...
                            img_path = f_path.replace("labels", "images").replace(
                                ".txt", ".jpg"
                            )
            ...
            messagebox.showinfo(
                "Éxito", f"Objeto '{category}' eliminado correctamente.", parent=self
            )
    ...
    def confirm(self):
        ...
        if not new_mapping:
            messagebox.showwarning(
                "Atención",
                "Debés seleccionar al menos un objeto para entrenar.",
                parent=self,
            )
            return
...
class SettingsDialog(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        header = ctk.CTkLabel(
            self, text="Configuracion Maestra", font=ctk.CTkFont(size=20, weight="bold")
        )
        ...
        self.pass_entry = self.create_input(
            frame_red,
            "Contraseña SSH:",
            self.config_data.get("contrasena", "12345678"),
            show="*",
        )
        ...
        self.lbl_ping_status = ctk.CTkLabel(
            frame_red, text="Estado: Desconocido", font=ctk.CTkFont(size=11)
        )
        ...
        ctk.CTkLabel(
            frame_ia,
            text="PARÁMETROS DE INTELIGENCIA",
            font=ctk.CTkFont(size=12, weight="bold"),
            text_color="#F57C00",
        ).pack(pady=5)
        ...
        self.epochs_entry = self.create_input(
            ...
            str(self.config_data.get("epochs", 300)),
        )
        ...
        self.lbl_conf_val = ctk.CTkLabel(
            self.conf_frame, text=f"Confianza Mínima: {conf_inicial:.2f}"
        )
        ...
        self.conf_slider = ctk.CTkSlider(
            ...
            from_=0.1,
            to=1.0,
            command=lambda val: self.lbl_conf_val.configure(
                text=f"Confianza Mínima: {val:.2f}"
            ),
        )
    ...
    def save_config(self):
        try:
            ...
            messagebox.showerror(
                "Error",
                "Por favor ingresá valores numéricos válidos en Épocas y Confianza.",
            )

    def run_network_scan(self):
        self.lbl_ping_status.configure(
            text="Estado: Buscando raspberrypi.local...", text_color="yellow"
        )
        ...
        def sc_thread():
            try:
                ...
                self.lbl_ping_status.configure(
                    text=f"Estado: Encontrada! IP: {ip}", text_color="#4CAF50"
                )
            except socket.gaierror:
                self.lbl_ping_status.configure(
                    text="Estado: No encontrada en la red local. Ingrese IP Manual.",
                    text_color="#F44336",
                )
    ...
    def run_ping_test(self):
        ...
        self.lbl_ping_status.configure(
            text="Estado: Verificando...", text_color="white"
        )
        self.update_idletasks()
        ...
        def ping():
            try:
                ...
                if result.returncode == 0:
                    self.after(
                        0,
                        lambda: self.lbl_ping_status.configure(
                            text="Estado: ONLINE", text_color="#4CAF50"
                        ),
                    )
                else:
                    self.after(
                        0,
                        lambda: self.lbl_ping_status.configure(
                            text="Estado: OFFLINE", text_color="#F44336"
                        ),
                    )
                ...
                self.after(
                    0,
                    lambda m=err_msg: self.lbl_ping_status.configure(
                        text=f"Error: {m}", text_color="#F44336"
                    ),
                )
...
class MLOpsPanel(ctk.CTk):
    def __init__(self):
        ...
        if os.path.exists(path_logo_texto):
            try:
                ...
                img_ctk = ctk.CTkImage(
                    light_image=img_pil, dark_image=img_pil, size=(200, 50)
                )
                self.logo_label = ctk.CTkLabel(
                    self.sidebar_frame, text="", image=img_ctk
                )
            except (IOError, AttributeError):
                self.logo_label = ctk.CTkLabel(
                    self.sidebar_frame,
        ...
        self.btn_ingest = ctk.CTkButton(
            self.sidebar_frame,
            text="1. Nuevo entrenamiento",
            command=self.run_ingest,
            fg_color="#1E88E5",
            hover_color="#1565C0",
            font=ctk.CTkFont(size=14, weight="bold"),
            height=35,
        ...
        if os.path.exists(path_icono_central):
            try:
                ...
                img_ic_ctk = ctk.CTkImage(
                    light_image=img_ic, dark_image=img_ic, size=(60, 60)
                )
                self.icono_label = ctk.CTkLabel(
                    self.sidebar_frame, text="", image=img_ic_ctk
                )
        ...
        self.perf_frame = ctk.CTkFrame(
            self.sidebar_frame, fg_color="#222222", corner_radius=5
        )
        ...
        self.btn_factory = ctk.CTkButton(
            self.main_frame,
            text="Resetear de Fabrica",
            command=self.run_reset_all,
            fg_color="#D32F2F",
            hover_color="#C62828",
            font=ctk.CTkFont(size=12, weight="bold"),
            width=130,
        ...
        ctk.CTkLabel(
            self.footer_frame,
            text="Creado por: Medina Albaro y Asencio Issaias | Contacto: ",
            font=ctk.CTkFont(size=11, slant="italic"),
            text_color="#AAAAAA",
        ).pack(side="left")
        lbl_email1 = ctk.CTkLabel(
            self.footer_frame,
            text="medinaferesalbaro@gmail.com",
            cursor="hand2",
            font=ctk.CTkFont(size=11, slant="italic", underline=True),
            text_color="#1E88E5",
        )
        lbl_email1.pack(side="left")
        lbl_email1.bind(
            "<Button-1>",
            lambda e: webbrowser.open("mailto:medinaferesalbaro@gmail.com"),
        )
        ctk.CTkLabel(
            self.footer_frame,
            text=" - ",
        ...
        lbl_email2.bind(
            "<Button-1>", lambda e: webbrowser.open("mailto:issaiasasencio@gmail.com")
        )
            ...
            try:
                ...
                if accion == "detener":
                    self.after(
                        0,
                        lambda: self.log(
                            "[REMOTO] Solicitando liberación de cámara en Raspberry Pi..."
                        ),
                    )
                    ...
                    self.after(
                        0,
                        lambda: self.log(
                            "[REMOTO] Reanudando servicios de escaneo en Raspberry Pi..."
                        ),
                    )
                    # Corregido a .venv según se ve en tu terminal
                    ssh.exec_command(
                        "nohup /home/pi/Desktop/Flex-Sort/.venv/bin/python /home/pi/Desktop/Flex-Sort/rpi_panel.py > /dev/null 2>&1 &"
                    )
                ssh.close()
            except Exception as e:
                self.after(
                    ...
                    lambda err=e: self.log(
                        f"[REMOTO] Aviso: No se pudo {accion} el escaneo. Error: {str(err)}"
                    ),
                )
        ...
        if confirm:
            ...
            def task():
                ...
                if success:
                    ...
                    self.after(
                        0, lambda: messagebox.showinfo("Éxito", msg_exito, parent=self)
                    )
                    self.after(
                        0,
                        lambda: self.log(
                            "[CALIBRACION] Fondo maestro listo para futuros entrenamientos."
                        ),
                    )
                else:
                    self.after(
                        ...
                        lambda: messagebox.showerror(
                            "Error",
                            "No se pudo realizar la calibracion remota. Chequea los logs.",
                            parent=self,
                        ),
                    )
                    self.after(
                        0, lambda: self.log("[ERROR] Falla en la calibracion de fondo.")
                    )
    ...
    def create_perf_bar(self, parent, label, color):
        ...
        bar = ctk.CTkProgressBar(
            frame, height=8, progress_color=color, fg_color="#333333"
        )
        bar.set(0)
    ...
    def run_reset_all(self):
        ...
        files_to_delete = [
            "Proyecto_FlexSort/dataset/data.yaml",
            "Proyecto_FlexSort/dataset/servo_mapping.json",
        ]
        ...
        def on_finish_ingest():
            def check_bg_calibration():
                ruta_fondo = os.path.join(
                    "Proyecto_FlexSort", "recursos", "fondo_maestro"
                )
                ...
                if os.path.exists(ruta_fondo):
                    for f in os.listdir(ruta_fondo):
                        if f.endswith(".jpg"):
                            ...
                            break
                if fondo_detectado:
                    msg = "¡OBJETO GUARDADO! Además, se detectó tú último Fondo Maestro (cinta vacía) y se aplicó con éxito.\n\n¿Deseás grabar un fondo maestro NUEVO ahora?"
                else:
                    msg = "¡OBJETO GUARDADO! Sin embargo...\n\nNo se detectó un Fondo Maestro de tu cinta vacía.\n¿Deseás grabar el fondo vacío ahora?"
    ...
    def run_train(self):
        ...
        def suggest_report():
            if messagebox.askyesno(
                ...
                "¿Deseás ver el Reporte de Calidad del nuevo modelo ahora?",
            ):
                self.open_report()

        self.run_subprocess(
            [self.python_exe, "3_entrenar_modelo.py", dialog.result],
    ...
    def run_infer(self):
        ...
        if not modelo_path:
            ...
            if pt_files:
                ...
                if messagebox.askyesno(
                    "Historial",
                    f"¿Activar último modelo entrenado?\n{os.path.basename(latest)}",
                ):
                    modelo_path = latest
        if not modelo_path:
    ...
    def run_optimize(self):
        ...
        if not modelo_path:
            ...
            if pt_files:
                ...
                if messagebox.askyesno(
                    "Optimización",
                    f"¿Optimizar último modelo entrenado?\n{os.path.basename(latest)}",
                ):
                    modelo_path = latest

        if not modelo_path:
            modelo_path = filedialog.askopenfilename(
                title="Seleccionar Modelo para Optimizar",
                filetypes=[("Modelos YOLO", "*.pt")],
            )

        if modelo_path:
            self.run_subprocess(
                [self.python_exe, "5_optimizar_modelo.py", modelo_path, "ncnn"]
            )
    ...
    def run_deploy(self):
        ...
        self.run_subprocess(
            [self.python_exe, "6_enviar_a_raspberry.py", modelos_ncnn[0]]
        )
