...
class MLOpsPanel(ctk.CTk):
        ...
        for r, ds, fs in os.walk(os.getcwd()):
            ...
            for d in list(ds):
                if d.endswith("_ncnn_model"):
                    try: shutil.rmtree(os.path.join(r, d))
                    except: pass
    ...
    def run_optimize(self):
        ...
        if pt_files:
            ...
            respuesta = messagebox.askyesno("Optimizar Modelo", f"Se detectó tu último modelo PyTorch:\n...{latest[-50:]}\n\n¿Quieres usar este base?\n\n- SÍ = Autodetectado\n- NO = Seleccionar manualmente.")
            if respuesta:
                modelo_path = latest

        if not modelo_path:
            modelo_path = filedialog.askopenfilename(title="Seleccionar Modelo (.pt) a Optimizar", filetypes=[("Modelos PyTorch", "*.pt")])
            if not modelo_path:
                return
                
        formato = simpledialog.askstring("Formato de Exportación", "¿A qué formato ligero deseas exportar?\n\nOpciones principales:\n- 'ncnn' (Máximo rendimiento en Rasp Pi, exporta como CARPETA)\n- 'tflite' (Muy bueno en Rasp Pi, exporta ARCHIVO)\n\nEscribe ncnn o tflite:", initialvalue="ncnn")
    ...
    def run_deploy(self):
        ...
        self.log(f" >>> Iniciando envío directo al Edge (Raspberry Pi)...")
